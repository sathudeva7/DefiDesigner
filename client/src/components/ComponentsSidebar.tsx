...
const ComponentsSidebar = ({ editor }: ComponentsSidebarProps) => {
  ...
  useEffect(() => {
    ...
    const setupDragDrop = () => {
      try {
        ...
        // Add a class to the document during drag to style
        ...
        // Make sure components are categorized properly
        editor.on('load', () => {
          // Force a refresh to make sure block categories are shown
          setTimeout(() => {
            refreshBlocks();
          }, 500);
        });
  ...
  // Refreshes blocks from the editor
  const refreshBlocks = () => {
    if (!editor) return;
    ...
    editor.BlockManager.getAll().each((block: any) => {
      ...
      if (typeof blockCategory === 'object' && blockCategory !== null) {
        categoryName = blockCategory.label;
      } else if (typeof blockCategory === 'string') {
        categoryName = blockCategory;
      }
      
      // Add to appropriate category if it exists in our structure
      if (newBlocks[categoryName]) {
        newBlocks[categoryName].push(block);
      }
    });
  ...
  // Create a draggable block item component
  const createDraggableBlockItem = (id: string, label: string, content: string, icon: string = "C") => {
    return (
      <div 
        key={id}
  ...
  // Render blocks from the editor or fallback to hardcoded ones
  const renderBlocks = (category: string) => {
    if (!editor) return null;
    
    // If we have blocks from the editor for this category, render them
    if (blocks[category] && blocks[category].length > 0) {
      return blocks[category].map((block: any) => {
        ...
        return (
          <div 
            ...
            onClick={() => {
              if (editor) {
                editor.addComponents(`<div data-gjs-type="wrapper">${blockContent}</div>`);
              }
            }}
          >
            <div className="w-8 h-8 bg-panel-bg rounded flex items-center justify-center text-primary mr-2"
              dangerouslySetInnerHTML={{ __html: blockMedia }}
            />
    ...
    // Otherwise, render hardcoded blocks
    if (category === "Basic") {
      return (
        <>
          {createDraggableBlockItem("text", "Text", '<div data-gjs-type="text">Insert your text here</div>', "T")}
          {createDraggableBlockItem("image", "Image", '<img src="https://via.placeholder.com/350x150" alt="Placeholder" />', "I")}
          {createDraggableBlockItem("button", "Button", '<button style="padding: 10px 20px; background-color: #6e56cf; color: white; border: none; border-radius: 4px;">Click Me</button>', "B")}
        </>
      );
    }
    ...
    if (category === "DeFi Components") {
      return (
        <>
          {createDraggableBlockItem("connect-wallet-button", "Connect Wallet Button", 
            ...
            "W")}
          {createDraggableBlockItem("token-swap-widget", "Token Swap Widget", 
            ...
            "S")}
          {createDraggableBlockItem("lending-pool-panel", "Lending Pool Panel", 
            ...
            "L")}
          {createDraggableBlockItem("yield-farming-section", "Yield Farming Section", 
    ...
    if (category === "Layouts") {
      return (
        <>
          {createDraggableBlockItem("hero-section", "Hero Section", 
            ...
            "H")}
          {createDraggableBlockItem("two-columns", "Two Columns", 
            ...
            "2C")}
          {createDraggableBlockItem("card-grid", "Card Grid", 
